...
const getScaleFactor = (position, viewportWidth) => {
    ...
    if(position < 0 || position > viewportWidth){
        ...
        return lerp(0.45, 1.5, (position - 2 * quaterWidth) / quaterWidth);
    }else{
...
const update = () => {
    ...
    slider.style.transform = `translate(${currentX})`;
    updateScales();
    requestAnimationFrame(update);
}

window.addEventListener("scroll", () => {
    const maxScorll = document.body.scrollHeight - window.innerHeight;
    const scrollProgress = window.scrollY / maxScorll;
